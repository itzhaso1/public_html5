        ...
        <div class="mb-5 card card-xxl-stretch mb-xl-8">
            <div class="pt-5 border-0 card-header">
                <h3 class="card-title align-items-start flex-column">
                    <span class="mb-1 card-label fw-bolder fs-3">{{ $pageTitle ?? 'إضافة صور للسلايدر' }}</span>
                    <span class="mt-1 text-muted fw-bold fs-7">يمكنك رفع صورة واحدة أو عدة صور دفعة واحدة.</span>
                </h3>
            </div>
            ...
            <div class="py-3 card-body">
                <form action="{{ route('admin.sliders.store') }}" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-8">
                                        ...
                                        <textarea name="{{ $key }}[description]" rows="3"
                                                  class="form-control">{{ old($key.'.description') }}</textarea>
                                    </div>
                                </div>
                            </div>
                        @endforeach
                    ...
                    <div class="row">
                        <div class="col-md-8">
                            <label class="form-label fw-bold">صور السلايدر</label>
                            <input class="form-control" type="file" name="sliders[]" multiple
                                   accept="image/jpeg,image/png,image/webp">
                            <div class="form-text">
                                يدعم: JPG / PNG / WEBP — يمكنك اختيار أكثر من صورة.
                            </div>
                            @error('sliders')<div class="text-danger small mt-1">{{ $message }}</div>@enderror
                            @error('sliders.*')<div class="text-danger small mt-1">{{ $message }}</div>@enderror
                        </div>
                    </div>
